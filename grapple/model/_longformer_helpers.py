...
class LongformerSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        attention_band = config.attention_band
        assert (
            attention_band % 2 == 0
        ), f"`attention_band` for layer has to be an even value. Given {attention_band}"
        assert (
            attention_band > 0
        ), f"`attention_band` for layer has to be positive. Given {attention_band}"

        self.one_sided_attn_window_size = attention_band // 2
    ...
    def forward(
        ...
        # apply dropout
        attn_probs = F.dropout(attn_probs, p=self.config.attention_probs_dropout_prob, training=self.training)
    ...
    def _compute_global_attn_output_from_hidden(
        ...
        global_attn_probs = F.dropout(
            global_attn_probs_float.type_as(global_attn_scores), p=self.config.attention_probs_dropout_prob, training=self.training
        )
