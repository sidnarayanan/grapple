...
""" Re-implementation of ALBERT
    with support for band-diagonal attention

    Papu: sparse attention for graph regression
...
class PapuAttention(BertSelfAttention):
    def __init__(self, config):
        ...
        self.adj_width = config.adj_width
...
class PapuLayer(nn.Module):
    def __init__(self, config):
        super().__init__()

        self.config = config
        self.full_layer_layer_norm = nn.LayerNorm(config.hidden_size, eps=config.layer_norm_eps)
        self.attention = PapuAttention(config)
...
class PapuLayerGroup(nn.Module):
    def __init__(self, config):
        super().__init__()

        self.output_attentions = config.output_attentions
        self.output_hidden_states = config.output_hidden_states
        self.albert_layers = nn.ModuleList([PapuLayer(config) for _ in range(config.inner_group_num)])
...
class PapuTransformer(nn.Module):
    def __init__(self, config):
        super().__init__()
        ...
        self.albert_layer_groups = nn.ModuleList([PapuLayerGroup(config) for _ in range(config.num_hidden_groups)])
...
class Papu(nn.Module):
    def __init__(self, config):
        ...
        self.encoders = nn.ModuleList([PapuTransformer(config) for _ in range(config.num_encoders)])
